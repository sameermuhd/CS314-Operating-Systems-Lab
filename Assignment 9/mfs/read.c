...
static void removeInodeEntry(struct inode *rip);	/* Assignment 9 */
...
int fs_readwrite(void)
{
  ...
  regular = (mode_word == I_REGULAR || mode_word == I_NAMED_PIPE || mode_word==I_IMMEDIATE);  /* Assignment 9 */
...
/* Assignment 9 */
  if(mode_word == I_IMMEDIATE) {
    if(rw_flag == WRITING) {
      if(position + nrbytes > 32) {
        ...
        int post = 0;
        char* temp_bytes;
        char buffer[40];	// Max 40 bytes because 10 u32 i_zones are present
        register struct buf* bp;
        
        // Copying file data in i_zones to buffer.
        for(i = 0; i < f_size; ++i) {
          if(i%4 == 0)
  ...
  if(immediate == 1) {
    if(rw_flag == READING) {
      ...
      printf("Minix: Contents of Immediate File:\n");
      for(i = 0; i < f_size; ++i) {
        printf("%c", buffer[i]);
      }
      printf("Minix: End of Immediate File\n");
...
unsigned bytes_ahead;		/* bytes beyond position for immediate use */ /* Assignment 9 */
{
